...
sampler uImage1 : register(s1);//扰动贴图
sampler uImage2 : register(s2);//消融贴图
...
float4 PixelShaderFunction(PSInput input) : COLOR0 {
    ...
    float2 resultCoord = float2(xx, yy) + uTime;
    ...
    if (dissolveTime > 0)
    {
        ...
        float4 disTex = tex2D(uImage2, frac(float2(xx, yy) + uTime3)); //是否加uTime?
        if (disTex.r < dissolveTime2 || dissolveTime2 >= 1)
            return float4(0, 0, 0, 0);
		
        dissolveAlpha = smoothstep(0, dissolveEdge, disTex.r - dissolveTime2);
        dissolveAlpha = clamp(dissolveAlpha, 0, 1);

    }
	
    if (strength > 0)
    {
        float4 noise = tex2D(uImage1, float2(float2(xx, yy) + uTime2));
        float2 move = noise.rg * strength;
        resultCoord += move;

    }
	
    return tex2D(uImage0, resultCoord) * coord.z * input.Color * dissolveAlpha;
