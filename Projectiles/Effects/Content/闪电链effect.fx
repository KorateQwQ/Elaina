...
float4 color;
float rand;
float4 PSFunction(float2 coords : TEXCOORD0) : COLOR0 //将image0进行曲线变换
{
    float y = coords.y;
    ...
    float3 result = lerp(color.rgb, float3(1, 1, 1), image.r);
    //float4(color.rgb * image.r, color.a);
    //result.a = lerp(1, 0, abs(0.5 - y) / 0.5); //smoothstep(1, 0, abs(0.5 - y) / 0.5);

    return float4(result, image.r * color.a * smoothstep(0.7, 0, abs(0.5 - y) / 0.5)); //
}
