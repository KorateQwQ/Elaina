...
float4 PSFunction2(float2 coords : TEXCOORD0) : COLOR0 //通过uImage1,也就是noise的纹理去扰动uImage0
{
    float4 noise = tex2D(uImage1, float2(coords.x + uTime, coords.y + uTime));
    float2 move = noise.rg * strength;
    float streath2 = smoothstep(0.5, 0, abs(0.5 - coords.y));
...
float4 PSFunction3(float2 coords : TEXCOORD0) : COLOR0 //通过uImage1,也就是noise的纹理去扰动uImage0
{
    float4 noise = tex2D(uImage1, float2(coords.x + uTime, coords.y + uTime));
